...
!   Generate random realization from purly gaussian map
  ...
  SUBROUTINE ReadWMAP_map()
    ...
    REAL(DP), DIMENSION(:,:), ALLOCATABLE   :: map_beam
    ...
    IF (do_smooth) THEN
       ...
       CALL DGEMM('N','N',npix_fits,1,npix_fits,1.0d0,map_beam,npix_fits,map(:,1),npix_fits,0,signal,npix_fits)
       map(:,1) = signal(:)
       DEALLOCATE(signal)
    ENDIF
    ...
    ! wmap_npp = map_beam x transpose(map_beam)
       call DSYRK('L', 'N', npix_cut, npix_fits, 1.0d0, map_beam, npix_fits, &
                  0.0d0, wmap_npp, npix_cut)
    ENDIF
    
    ! write(0,'(2(e10.5,1x))') (wmap_noise(i),wmap_npp(i,i),i=0,npix_cut-1)
    ! Deallocate read-in arrays
    DEALLOCATE(map,map_noise,map_mask,map_beam)
    RETURN
