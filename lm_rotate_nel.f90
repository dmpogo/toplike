...
!$OMP END PARALLEL DO
    !DEALLOCATE(dlmm)
!NELSON
       write(0,*)"starting splot"
       allocate(CTpp_cplm_nel(0:lmax*(lmax+2),0:npix_fits-1))
       allocate(cplm(0:lmax*(lmax+2),0:lmax*(lmax+2)))
       write(0,*)'AAAAAAAAAAAAAAAAAAAAAAA'
       OPEN(file='clmplot.out',status='unknown',unit=666)
       write(0,*)'BBBBBBBBBBBBBBBBBBBBBBB'
       call getcplm(CTpp_cplm_nel,CTpp_evec_rotated,nside,lmax,w8ring)
       !write(0,*)'CCCCCCCCCCCCCCCCCCCCCCC'
       !call cgemm('N','C',lmax*(lmax+2)+1,lmax*(lmax+2)+1,npix_fits,(1.0d0,0.0d0),CTpp_cplm_nel,lmax*(lmax+2)+1,CTpp_cplm_nel,lmax*(lmax+2)+1,(0.0d0,0.0d0),cplm,lmax*(lmax+2)+1)
       !write(0,*)'DDDDDDDDDDDDDDDDDDDDDDD'
       do i=0,lmax*(lmax+2)
       !  do j=0,lmax*(lmax+2)
         do j=0,npix_fits-1
       !    if(aimag(cplm(i,j))/=0) then
       !       write(0,*)"imaginary",i,j,aimag(cplm(i,j))
       !    endif
           write(666,*)real(CTpp_cplm_nel(i,j)), aimag(CTpp_cplm_nel(i,j))
         enddo
       enddo
       write(0,*)'EEEEEEEEEEEEEEEEEEEEEEE'
       deallocate(CTpp_cplm_nel)
       deallocate(cplm)
       write(0,*)'FFFFFFFFFFFFFFFFFFFFFFF'
       close(666)
       write(0,*)'GGGGGGGGGGGGGGGGGGGGGGG'
       write(0,*)"done splot"
!NELSON
